...
#Vendo se os dados estão em data frame
is.data.frame(dados_atividade)
...
sd(dados$`Altura(cm)`)
sd(dados$`Peso(kg)`)
glimpse(dados) # saber quais os tipos das variáveis

#Construindo o modelo de regressão linear(PESO DEPENDE DA ALTURA?)
#peso = b0 + b1*altura

linearMod <- lm(`Peso(kg)`~ `Altura(cm)`, data=dados)  # faz o modelo de regressão linear
print(linearMod) #o intercepto(beta0) e o beta 1
summary(linearMod)

# peso = -76.51 + 0.82*altura
...
cor(dados$Altura_cm,dados$Peso_kg)
